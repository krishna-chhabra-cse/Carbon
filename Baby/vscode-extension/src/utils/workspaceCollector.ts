...
export interface CollectedFile {
  path: string;       // Normalized POSIX relative path, e.g. "src/index.ts"
...
const MAX_TOTAL_CHARS        = 100_000;       // 100k characters for LLM context
 ...
 */
function isEligibleCodeFile(filename: string, ext: string): boolean {
 ...
 */
async function getGitignorePatterns(rootUri: vscode.Uri): Promise<string[]> {
  try {
 ...
 */
function buildFolderStructure(relativePaths: string[], workspaceName: string): string {
  const lines: string[] = [`[DIR] ${workspaceName}/`];
...
export async function collectWorkspaceFiles(
  ...
  if (onProgress) onProgress('Scanning workspace files...');
  ...
  const eligibleFiles: { uri: vscode.Uri; relPath: string; ext: string; size: number }[] = [];
  ...
  if (onProgress) onProgress(`Reading ${Math.min(eligibleFiles.length, MAX_TOTAL_FILES)} code files...`);
 ...
 */
function getFilePriority(relPath: string): number {
  const filename = path.basename(relPath).toLowerCase();
  const depth = relPath.split('/').length;

  if (filename === 'package.json' || filename === 'requirements.txt' || filename === 'cargo.toml' || filename === 'go.mod') return 1;
  if (filename.includes('server') || filename.includes('main') || filename.includes('app') || filename.includes('index')) return 2;
  if (filename.includes('docker') || filename.includes('config')) return 3;
  if (depth <= 2) return 4;
  return 5;
}
